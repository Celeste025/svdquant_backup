#!/usr/bin/env bash
# Wan2.1-1.3B s16 PTQ with gated OutputsError (attn1/FFN * gate).
# Reuses the same 16-prompt collect caches; writes a new ckpt for A/B vs ungated s16.
...
# Prefer this repo's deepcompressor (gated OutputsError changes).
export PYTHONPATH="${REPO_ROOT}/third_party/deepcompressor${PYTHONPATH:+:${PYTHONPATH}}"

CALIB_DIR="${DATA_ROOT}/datasets/torch.bfloat16/wan2.1-1.3b/unipc50-g6.0-f33/vbench/s16"
RUN_ROOT="${DATA_ROOT}/runs/wan_s16_gated"
SAVE_DIR="${DATA_ROOT}/ckpts/wan2.1-1.3b-int4-s16-gated"
...
echo "=== $(date -Iseconds) Wan2.1-1.3B s16 GATED PTQ ==="
echo "CALIB_DIR=${CALIB_DIR}"
echo "SAVE_DIR=${SAVE_DIR}"
echo "backup tag: backup-pre-gate (see docs/BACKUP_PRE_GATE.md)"
nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv || true

python "${REPO_ROOT}/scripts/check_wan_attn_struct.py"
python "${REPO_ROOT}/scripts/check_wan_gated_wrap.py"
...
python "${REPO_ROOT}/scripts/_deepcompressor_entry.py" deepcompressor.app.diffusion.ptq \
  ...
  --skip-gen \
  --eval-num-gpus 1 \
  2>&1 | tee "${LOG_DIR}/wan_s16_gated_ptq.log"
