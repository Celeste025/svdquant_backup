#!/usr/bin/env bash
# Wan2.1-1.3B collect: 16 VBench prompts x 50 steps (CFG x2 => ~1600 model-level caches).
set -euo pipefail
...
cd "${DIFFUSION}"
echo "=== $(date -Iseconds) Wan2.1-1.3B s16 collect ==="
...
python "${REPO_ROOT}/scripts/_deepcompressor_entry.py" deepcompressor.app.diffusion.dataset.collect.calib \
  ...
  configs/collect/vbench_s16.yaml \
  --eval-num-gpus 1 \
  --eval-batch-size 1 \
  2>&1 | tee "${LOG_DIR}/wan_s16_collect.log"
