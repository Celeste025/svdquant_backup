#!/usr/bin/env python3
"""Collect Linear-INPUT absmax surfaces: raw vs postsmooth (multilayer probes).

Same layer set as Fig.1 multilayer. Records per forward call
  raw[c]         = max_tokens |x[..., c]|
  postsmooth[c]  = max_tokens |(x + shift) / s|
then CFG-max aggregates and stores log2(·.clamp_min(2**-20)).
"""
...
def register_hooks(
    ...
    for key, info in metadata.items():
        ...
        def hook(_module, inputs, *, key=key, smooth=smooth, shift=shift):
            ...
            raw_records[key].append(flat.abs().amax(dim=0))
            xs = flat + shift if shift else flat
            xs = xs / smooth.cpu().to(dtype=xs.dtype)
            postsmooth_records[key].append(xs.abs().amax(dim=0))

        handles.append(module.register_forward_pre_hook(hook))
    return handles


def stack_log2(records: list[torch.Tensor], calls_per_step: int) -> torch.Tensor:
    ...
    return values.clamp_min(ABS_EPS).log2()
...
def collect(model: str, output_dir: Path) -> None:
    ...
    surfaces_ps = {}
    for key in metadata:
        surfaces_raw[key] = stack_log2(raw_records[key], calls_per_step)
        surfaces_ps[key] = stack_log2(postsmooth_records[key], calls_per_step)
    ...
    payload = {
        "model": model,
        "definition": (
            "log2(max_CFG max_tokens |·|); raw = x, postsmooth = (x+shift)/s "
            f"on Linear INPUT; abs floor={ABS_EPS}"
        ),
        "steps": 50,
        "seed": 44,
    ...
    path = output_dir / f"{model}_multilayer_absmax_input_surfaces.pt"
    torch.save(payload, path)
    (output_dir / f"{model}_multilayer_absmax_input_metadata.json").write_text(
        json.dumps(
            {
                "model": model,
                "path": str(path),
    ...
    print(json.dumps({"model": model, "path": str(path), "layers": list(surfaces_raw)}, indent=2))
