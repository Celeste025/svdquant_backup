#!/usr/bin/env bash
# Wan2.1-1.3B smoke PTQ: INT4 SVDQuant (svd_lowrank), grid10, batch4.
# Note: --calib-num-samples is # of .pt cache files (not # of prompts). Prefer run_wan_s16_ptq.sh.
set -euo pipefail
...
CALIB_DIR="${DATA_ROOT}/datasets/torch.bfloat16/wan2.1-1.3b/unipc50-g6.0-f33/vbench/s8"
RUN_ROOT="${DATA_ROOT}/runs/wan_smoke"
SAVE_DIR="${DATA_ROOT}/ckpts/wan2.1-1.3b-int4-smoke"
...
echo "=== $(date -Iseconds) Wan2.1-1.3B smoke PTQ ==="
...
python "${REPO_ROOT}/scripts/_deepcompressor_entry.py" deepcompressor.app.diffusion.ptq \
  ...
  --skip-gen \
  --eval-num-gpus 1 \
  2>&1 | tee "${LOG_DIR}/wan_smoke_ptq.log"
