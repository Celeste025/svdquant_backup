...
class DiffusionPipelineConfig:
    ...
    def _default_build(
        ...
        cpu_offload = os.environ.get("DEEPCOMPRESSOR_CPU_OFFLOAD", "")
        if cpu_offload == "sequential" and str(device).startswith("cuda"):
            gpu_id = torch.device(device).index or 0
            pipeline.enable_sequential_cpu_offload(gpu_id=gpu_id)
        elif cpu_offload == "1" and str(device).startswith("cuda"):
            gpu_id = torch.device(device).index or 0
